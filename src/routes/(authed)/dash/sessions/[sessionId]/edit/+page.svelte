...
<form class="flex flex-col gap-2 max-w-sm" method="POST" use:enhance>
	<p class="text-sm text-muted-foreground">Enter all dates and times in {data.sessionInfo.session.timezone}, where it's currently {DateTime.now().setZone(data.sessionInfo.session.timezone).toLocaleString(DateTime.TIME_SIMPLE)}</p>
	...
	<div class="flex flex-row gap-2">
		<Form.Field {form} name="hour">
			<Form.Control>
				...
				<Input onblur={pad} {...props} type="number" bind:value={$formData.hour} />
				{/snippet}
			</Form.Control>
		...
		<Form.Field {form} name="minute">
			<Form.Control>
				...
				<Input onblur={pad} {...props} type="number" bind:value={$formData.minute} />
				{/snippet}
			</Form.Control>
			...
			{#snippet children({props})}
				...
				<Select.Root type="single" bind:value={$formData.type} name={props.name}>
					<Select.Trigger {...props}>
						{$formData.type ? `${data.typesMap[$formData.type].name} (${data.typesMap[$formData.type].length} minutes)` : "Select a session type"}
					</Select.Trigger>
					<Select.Content>
	...
	{#if roleOf(data.user) >= ROLE_STAFF}
		<UserSelector label="Mentor" {form} usersMap={data.usersMap} name="mentor" bind:value={$formData.mentor} />
	{/if}
