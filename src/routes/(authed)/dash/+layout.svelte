...
<Sidebar.Provider>
	<DashSidebar role={roleOf(data.user)} userId={data.user.id} name="{data.user.firstName} {data.user.lastName}" />
	<main class="flex-1">
					...
					<Breadcrumb.Root>
						<Breadcrumb.List>
							{#each page.data.breadcrumbs as breadcrumb, i}
								...
								<Breadcrumb.Item>
									{#if breadcrumb.url}
										<Breadcrumb.Link class={last ? '' : 'hidden md:block'} href={breadcrumb.url}>{breadcrumb.title}</Breadcrumb.Link>
									{:else}
										<Breadcrumb.Page class={last ? '' : 'hidden md:block'}>{breadcrumb.title}</Breadcrumb.Page>
									{/if}
								</Breadcrumb.Item>
								{#if !last}
